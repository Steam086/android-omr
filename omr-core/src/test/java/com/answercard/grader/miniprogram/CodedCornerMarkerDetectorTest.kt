...
class CodedCornerMarkerDetectorTest {
    ...
    fun decodesFourUniqueMarkersAndReturnsTheirCenters() {
        ...
        assertTrue(match.diagnostics.componentCount >= 4)
        assertTrue(match.diagnostics.sizeAspectCandidateCount >= 4)
        assertTrue(match.diagnostics.decodedCandidateCount >= 4)
        assertEquals(CornerMarkerId.entries.toSet(), match.diagnostics.selectedEdgeLengths.keys)
        assertTrue(match.diagnostics.selectedEdgeLengths.values.all { it > 0.0 })
        assertTrue(match.diagnostics.debugInfo().any { it.startsWith("codedMarkerComponents=") })
        assertTrue(match.diagnostics.debugInfo().any { it.startsWith("codedMarkerSelectedEdgeLengths=") })
